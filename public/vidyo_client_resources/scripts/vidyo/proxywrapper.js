		...
		var videohandler = {
			...
			start: function(videoElement, callback) {
					...
					sourceInfos.forEach(function(source){
						if (source.kind === 'video' || source.kind === 'videoinput' ){
							...
							if(source.label === 'WebPluginVirtualCamera'){
								virtualCamerSource=source;
							}
						}
					});
					var getInitialVideoConstraints = function() {
						if (navigator.appVersion.indexOf('Windows') !== -1) {
							return {deviceId: virtualCamerSource ? {exact: virtualCamerSource.deviceId} : undefined, width: {exact: 1920}, height: {exact: 1080}};
						}
						return {deviceId: virtualCamerSource ? virtualCamerSource.deviceId : undefined, width: 1920, height: 1080};
					};
					var initialConstraints = {
						audio:false,
						video: getInitialVideoConstraints()
					};
					navigator.getUserMedia(initialConstraints, onStreamSuccess, onStreamErrors);
				}
				...
				if (navigator.mediaDevices && navigator.mediaDevices.enumerateDevices) {
					...
					MediaStreamTrack.getSources(getSourcesCallback);
				} else {
					return tempCallback('This browser does not support MediaDevices and MediaStreamTrack');
				}
