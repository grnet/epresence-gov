...
// ΣΥΝΑΡΤΗΣΕΙΣ ΣΧΕΤΙΚΕΣ ΜΕ DATATABLES
			...
			$(document).on("click", "[id^=RowBtnDelete]", function(){	
				$("#TeleModalMessage").hide();
...
// ΑΝΑΝΕΩΣΗ ΣΕΛΙΔΑΣ
    		...
    		var ShowDateTime = "{{trans('conferences.activeConferencesNow')}}: <strong>" 
					+ currentdate.getDate() + "/"
					+ (currentdate.getMonth()+1)  + "/" 
					+ currentdate.getFullYear() + "</strong> &amp; {{trans('conferences.time')}}: <strong>"  
					+ currentdate.getHours() + ":"  
					+ (currentdate.getMinutes()<10?'0':'')+currentdate.getMinutes() + "</strong>";	
			
			$('#TeleNow').html(ShowDateTime);		
				...
				<div class="row">
					<div class="col-sm-12">            
                        <ul class="nav nav-tabs">
                          ...
                          <li class="active"><a href="#">{{trans('conferences.ongoing')}}</a></li>
						  <li ><a href="/conferences/record">{{trans('conferences.history')}}</a></li>
...
<!-- DATATABLES START -->
				<div class="row">
                	<div class="col-md-12">
                		<h4 id="TeleNow" class="center alert alert-success"> {{trans('conferences.activeConferencesNow')}}: <strong> </strong> &amp; {{trans('conferences.time')}}: <strong> </strong></h4>
                    </div>
                </div>
				...
				<div class="row"> <!-- Row with search field and add button - START -->
					...
					<div class="col-md-7 col-sm-12 col-xs-12" style="text-align:right">
                    	<div>
                        	<button type="button" class="btn btn-default" style="padding-right:6px; padding-left:6px" id="RefreshPage">
                               <small> <span class="glyphicon glyphicon-refresh"></span> {{trans('conferences.refreshPage')}}</small>
                             </button>
                             
							 <button type="button" class="btn btn-default" style="padding-right:6px; padding-left:6px" id="InvEmail" data-toggle="tooltip" data-placement="top" title="{{trans('conferences.sendInfoEmail')}}">
                               <small> <span class="glyphicon glyphicon-envelope"></span> {{trans('conferences.sendEmail')}}</small>
                             </button>
                      </div>
					</div>					
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title" id="TeleModalLabel">{{trans('conferences.deleteConference')}}ς</h4>
					</div> <!-- .modal-header -->
					...
					<div class="alert alert-info" role="alert" id="TeleModalMessage">{{trans('conferences.conferenceDeleted')}} </div>
                        ...
                        <form id="OrgForm" class="form-horizontal" role="form">
                            ...
                            <div class="form-group">
                                <label for="FieldStartTime" class="control-label col-sm-3">{{trans('conferences.time')}}:</label>
                                <div class="col-sm-9">
        ...
        <div class="modal fade" id="InvEmailModal" tabindex="-1" role="dialog" aria-labelledby="InvEmailLabel" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                                <h4 class="modal-title" id="InvEmailLabel">{{trans('conferences.sendEmail')}}</h4>
                            </div> <!-- .modal-header -->
                            <div class="modal-body">
                            <div class="alert alert-info" role="alert" id="InvEmailMessage" >{{trans('conferences.emailSent')}} </div>
        
                                <form id="InvEmailForm" class="form-horizontal" role="form">
        
                                    <div class="form-group">
                                        <label for="FieldInvEmailFrom" class="control-label col-sm-1" style="text-align:left">{{trans('conferences.from')}}:</label>
                                        <div class="col-sm-11">
                                            <input class="form-control" id="FieldInvEmailFrom" placeholder="{{trans('conferences.fromExplain')}}" />
                                        </div>
                                    </div>
<div class="form-group">
                                        <label for="FieldInvEmailSubject" class="control-label col-sm-1" style="text-align:left">{{trans('conferences.subject')}}:</label>
                                        <div class="col-sm-11">
                                            <input class="form-control" id="FieldInvEmailSubject" placeholder="{{trans('conferences.sunjectExplain')}}"  />
                                        </div>
                                    </div>
        
                                    <div class="form-group" >
                            			<label for="FieldInvEmailTo" class="control-label col-sm-1" style="text-align:left">{{trans('conferences.recipient')}}:</label>
                            			<div class="col-sm-11 form-control-static">
                            				{!!trans('conferences.recipientExplain')!!} 
                            			</div>
                        			</div>                         
                                                                  
                    				<div class="form-group" >
                                    	<label for="FieldInvMessage" class="control-label col-lg-12" style="text-align:left">{{trans('conferences.message')}}:</label>
                                        	<div class="col-lg-12">
                                            	<div id="FieldInvMessage">{{trans('conferences.messageExplain')}}</div>
                                             </div>
                                  	</div>               
                            ...
                            <div class="modal-footer" style="margin-top:0px;">
                                <button  id="InvEmailSubmitBtn" class="btn btn-primary">{{trans('conferences.sendEmail')}}</button>                            
                                <button type="button" id="InvEmailModalButtonClose" class="btn btn-default" data-dismiss="modal">{{trans('conferences.cancel')}}</button>
