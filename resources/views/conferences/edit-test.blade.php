        ...
        <div class="container">
            ...
            <div class="box" style="padding:30px 30px  20px 30px">
                <div class="row" style="margin:0px;">
                    ...
                    @endif
                    {!! Form::model($conference, array('url' => ['test-conferences/' . $conference->id], 'method' => 'PATCH', 'class' => 'form-horizontal', 'id' => 'OrgForm', 'role' => 'form')) !!}
                    @include('conferences._testForm', ['apella_id'=>$conference->apella_id,'start_date' => $conference->getDate($conference->start), 'start_time' => $conference->getTime($conference->start), 'end_date' => $conference->getDate($conference->end), 'end_time' => $conference->getTime($conference->end),'invisible' => Form::getValueAttribute('invisible'), 'submitBtn' => trans('conferences.saveConference'), 'copyBtn' => ''])
                    {!! Form::close() !!}
                </div>
            </div><!--/.box-->
            <!-- Form Details -END -->
        ...
        @if($errors->isEmpty() || $errors->has('firstname') || $errors->has('lastname') || $errors->has('email') || $errors->has('institution_id') || $errors->has('department_id') || $errors->has('new_department') || $errors->has('new_institution')|| $errors->has('no_new_org') || $errors->has('state'))
            <!-- SYMMETEXONTES START -->
                ...
                <div class="box" id="ParticipatsBody">
                    ...
                    {{--<div class="row alert alert-info" role="alert" id="maxParticipanstNotification">--}}
                        {{--{{trans('conferences.participantLimitReached')}}--}}
                    {{--</div>--}}
